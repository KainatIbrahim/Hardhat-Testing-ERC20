...
describe("ERC20token2", function(){
    beforeEach(async function(){
        [owner, add1, add2] = await ethers. getSigners();
        const ERC20token2 = await ethers.getContractFactory("ERC20token2");//helps us to get the class for our contract
        token = await ERC20token2.deploy();
        await token.deployed();
    })
    describe("Token-Details", function(){
        it("it should return the name of token", async function(){
            let Name = await token.name();
            expect(Name).to.equal("TOKEN2");
            console.log("Token Name= ", Name);
        })
        it("it should return the tokensymbol of token", async function(){
            let Symbol = await token.symbol()
            expect(Symbol).to.equal("TK2");
    ...
    describe("Transfer" ,function(){
        ...
        expect(await token.balanceOf(add1.address)).to.equal(40);
